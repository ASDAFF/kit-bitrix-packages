...
<div id="sender-subscribe">
    ...
    <form id="bx_subscribe_subform_<?= $buttonId ?>" method="post" action="<?= $arResult["FORM_ACTION"] ?>" class="subscribe_subform">
        <div class="subscribe_subform__input">
            <?= bitrix_sessid_post() ?>
            <input type="hidden" name="sender_subscription" value="add">

            <input class="footer-block__follow_input_email" type="email"
                   name="SENDER_SUBSCRIBE_EMAIL" value="<?= $arResult["EMAIL"] ?>"
                   title="<?= GetMessage("subscr_form_email_title") ?>"
                   placeholder="<?= htmlspecialcharsbx(GetMessage('subscr_form_email_title')) ?>">
            <button class="footer-block__follow_input_submit" id="bx_subscribe_btn_<?= $buttonId ?>" title="<?= GetMessage("
                    subscr_form_button
            ") ?>">
        ...
        <div style="<?= ($arParams['HIDE_MAILINGS'] <> 'Y' ? '' : 'display: none;') ?>" class="subscribe-desc">
            <? if (count($arResult["RUBRICS"]) > 0): ?>
                <div class="subscribe-desc__title"><?= GetMessage("subscr_form_title_desc") ?></div>
            <? endif; ?>
    ...
    <form id="bx_subscribe_subform_<?= $buttonId ?>" method="post" action="<?= $arResult["FORM_ACTION"] ?>">
        ...
        <input class="footer-block__follow_input_email" type="email"
               ...
               title="<?= GetMessage("subscr_form_email_title") ?>"
               placeholder="<?= htmlspecialcharsbx(GetMessage('subscr_form_email_title')) ?>">
        <input type="submit" class="footer-block__follow_input_submit fonts__big_comment"
               id="bx_subscribe_btn_<?= $buttonId ?>"
               value="<?= GetMessage("NAME_BUTTON") ?>"
