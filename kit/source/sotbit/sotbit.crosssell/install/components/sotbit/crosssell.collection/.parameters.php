...
$arQuery = \Sotbit\Crosssell\Orm\CrosssellTable::getList(
    array(
        ...
        'filter' => array('TYPE_BLOCK' => 'COLLECTION', 'Active' => 'Y'),
    )
);
...
$db_collection_category = \Sotbit\Crosssell\Orm\CrosssellCategoryTable::getList(array(
    'select' =>
        array(
            'ID', 'NAME'
        ),
...
$arCollectionCategory = array();

while ($collection_item = $db_collection_category->fetch()){
    $arCollectionCategory[$collection_item['ID']] = $collection_item['NAME'];
}

$arCollection = array();

while ($item = $db_collection->fetch()){
    $arCollection[$item['CATEGORY_ID']][$item['ID']] = $item['NAME'];
}

$result = array();
foreach ($arCollection as $catId => $collections) {
    if ($arCollectionCategory[$catId] == "") {
        $categoryName = GetMessage('ROOT_CATEGORY');
    }
    else
    {
        $categoryName = $arCollectionCategory[$catId];
    }
    $result[$catId]['CATEGORY_NAME'] = $categoryName;
    $result[$catId]['ITEMS'] = array();
    if (is_array($collections)){
        foreach ($collections as $k => $v){
            $result[$catId]['ITEMS'][$k] = $v;
        }
    }
}

$arParametersCollection = array();
foreach ($result as $catId => $category) {
    $arParametersCollection["s" . $catId] = $category[CATEGORY_NAME];
    foreach ($category['ITEMS'] as $collectionId => $collectionName) {
        $arParametersCollection["e" . $collectionId] = " . " . $collectionName;
    }
}

$teplateDir = "/local/templates/.default/components/bitrix/catalog.section/";
$docRoot = \Bitrix\Main\Application::getDocumentRoot();
if (file_exists($docRoot . $teplateDir)){
    $templateDirs = scandir($docRoot . $teplateDir);
    if (is_array($templateDirs) && (count($templateDirs) > 0)){
        foreach ($templateDirs as $id => $file) {
            if (!is_dir($docRoot . $teplateDir . $file) || (in_array($file, array(".", "..")))){
                unset($templateDirs[$id]);
            }
        }
    }
    $templateDirs = array_values($templateDirs);
}
...
$collectionListTemplates = CComponentUtil::GetTemplatesList("sotbit:crosssell.collection.list", $_GET['template_id']);
$collectionListTemplatesArray = array();
foreach ($collectionListTemplates as $sectionTemplate) {
    $collectionListTemplatesArray[$sectionTemplate['NAME']] = $sectionTemplate['NAME'];
}
...
$arComponentParameters = array(
    ...
    'PARAMETERS' => array(
        ...
        'SECTION_TEMPLATE' => array(
            "PARENT" => "VISUAL",
            "NAME" => GetMessage("SECTION_TEMPLATE"),
            "TYPE" => "LIST",
            "VALUES" => $sectionTemplateArray
        ),
        'COLLECTION_LIST_TEMPLATE' => array(
            "PARENT" => "VISUAL",
            "NAME" => GetMessage("COLECTION_LIST_TEMPLATE"),
            "TYPE" => "LIST",
            "VALUES" => $collectionListTemplatesArray
        ),
        'COLLECTION_LIST' => array(
            "PARENT" => "BASE",
            "NAME" => GetMessage("COLLECTIONS"),
            "TYPE" => "LIST",
            "VALUES" => $arParametersCollection,
            "MULTIPLE" => "Y",
            "REFRESH" => "N",
            "SIZE" => "9",
        ),
