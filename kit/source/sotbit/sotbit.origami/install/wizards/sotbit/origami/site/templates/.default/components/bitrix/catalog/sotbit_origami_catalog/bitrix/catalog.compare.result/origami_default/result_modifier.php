...
$sectionProp = CIBlockSectionPropertyLink::GetArray($arParams['IBLOCK_ID'], 0);
foreach ($arResult['SHOW_PROPERTIES'] as $key => $property) {
    foreach ($sectionProp as $idProp => $contentProp) {
        if ($idProp == $property['ID']) {
            $arResult['SHOW_PROPERTIES'][$key]['FILTER_HINT'] = $contentProp['FILTER_HINT'];
        }
    }
}
