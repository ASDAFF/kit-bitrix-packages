...
<div class="puzzle_block news-block__wrapper main-container">
    <p class="puzzle_block__title fonts__middle_title">
        <?=$arParams["BLOCK_NAME"]?>
        <a href="<?=($arResult["ITEMS"][0]["LIST_PAGE_URL"]) ? $arResult["ITEMS"][0]["LIST_PAGE_URL"] : $arParams["LINK_TO_THE_FULL_LIST"]?>" title="<?=Loc::getMessage("SOTBIT_NEWS_TWO_BIG_LINK_TEXT")?>" class="puzzle_block__link fonts__small_text">
            <?=Loc::getMessage("SOTBIT_NEWS_TWO_BIG_LINK_TEXT");?>
            <i class="icon-nav_1"></i>
        </a>
    </p>

    <div class="news_block_two">
        <div class="news_block_two__wrapper">
            <?
            $i = 0;
            ?>
            <?foreach(array_slice($arResult["ITEMS"], 0, 6) as $arItem):?>
                ...
                ?>
                <?if($i <= 2):?>
                    <div class="news_block_two__item" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
