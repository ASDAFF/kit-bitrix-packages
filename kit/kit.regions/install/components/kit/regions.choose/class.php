 ...
 *
 */
class KitRegionsChooseComponent extends \CBitrixComponent
{
        ...
        if (Loader::includeModule('kit.regions'))
        {
            ...
            if ($this->arParams['FROM_LOCATION'] != 'Y' || !$this->arResult['USER_REGION_NAME'])
            {
                if (Option::get('SINGLE_DOMAIN', SITE_ID) != 'Y')
                {
                    ...
                    if ($_COOKIE['kit_regions_id'] > 0)
                    {
                        $this->arResult['USER_REGION_ID'] = $_COOKIE['kit_regions_id'];
                        foreach ($this->arResult['REGION_LIST'] as $region)
                        {
...
//                $this->arResult['ROOT_DOMAIN'] = $rDomain;
    ...
    {
        $return = true;
        if ($_COOKIE['kit_regions_city_choosed'] == 'Y') {
            $return = false;
        }
     ...
     * @throws \Bitrix\Main\SystemException
     */
    private function setRegionsList(\Kit\Regions\Location\Domain $domain)
    {
//        $mainDomain = RegionsTable::GetList(array(
//            'order'  => array("ID" => "asc"),
//            'select' => array("CODE"),
...
//        );
    ...
    private function setRegionsListFromLocation(
        \Kit\Regions\Location\Domain $domain
    ) {
        $return = [];
