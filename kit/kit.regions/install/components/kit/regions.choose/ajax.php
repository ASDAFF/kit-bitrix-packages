...
if (!Loader::includeModule('kit.regions') || !Loader::includeModule('sale') || !Loader::includeModule('main')
) {
    return false;
}
ob_start();
switch ($action)
{
    case 'getDomainByLocation':
        $Location = new \Kit\Regions\System\Location();
        $region = $Location->findRegionByIdLocation($id);
        if (!$region && !empty($id)) {
            $Region = new \Kit\Regions\Internals\RegionsTable();
            $region = $Region->getById($_REQUEST['id'])->fetch();
        }
        if(!$region)
        {
            $Region = new \Kit\Regions\Region();
