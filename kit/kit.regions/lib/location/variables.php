...
class Variables
{
    ...
    public static function getInstance()
    {
        if (null === self::$instance) {
            ...
            if ($_SESSION['KIT_REGIONS']
                && is_array($_SESSION['KIT_REGIONS'])
            ) {
                self::$instance->setCodes(array_keys($_SESSION['KIT_REGIONS']));
            }
        }
    ...
    public function getValues($codes = [], $lid = SITE_ID)
    {
        ...
        if ($codes)
        {
            ...
            {
                if (!isset($_SESSION['KIT_REGIONS'][$code]))
                {
                    continue;
                }

                $value = $_SESSION['KIT_REGIONS'][$code];

                if($code == 'NAME' && isset($_SESSION['KIT_REGIONS']['NAME']) &&
                    !empty($_SESSION['KIT_REGIONS']['NAME']))
                    $value = $_SESSION['KIT_REGIONS']['NAME'];
                ...
                $return[\KitRegions::genCodeVariable($code)] = $value;
            }
        }
    ...
    private function findVariables($text = '')
    {
        ...
        foreach ($this->getCodes() as $code) {
            $tag = \KitRegions::genCodeVariable($code);
            if(!empty($tag)) {
