...
class Facet
{
			...
			if (empty($where))
			{
			    ...
			    $sqlUnion[] = "
					...
					WHERE
                        ...
                        .(($this->sectionsId && is_array($this->sectionsId)) ? ' AND (F.SECTION_ID IN ('.implode(',', $this->sectionsId).'))' : '')
                        .(($this->elementsId && is_array($this->elementsId)) ? ' AND (F.ELEMENT_ID IN ('.implode(',', $this->elementsId).'))' : '')
                        ."
					GROUP BY
						F.FACET_ID, F.VALUE
				";
			...
			else
			{
				...
				$fcJoin = "
						INNER JOIN (
							...
							WHERE FC0.SECTION_ID IN (".implode(',', $this->sectionsId).")
							AND (
	...
	{
        $facetFilter = array();
        foreach($this->sectionsId as $sectId)
        {
            foreach($this->getSectionFilterProperty($sectId) as $propertyId => $propertyType)
            {
    ...
    protected function getFacetFilterOld(array $facetTypes)
    {
        $facetFilter = array();
        //foreach($this->sectionId as $sectId)
        {
	...
	public function whereToSql(array $where, $tableAlias, $subsectionsCondition = "")
	{
        ...
        if($this->sectionsId)
            $sectionCondition = "$tableAlias.SECTION_ID IN (".implode(',', $this->sectionsId).')';
        else
            $sectionCondition = "";
    ...
    public function setElementsId($elementIDs)
    {
        $this->elementsId = array_filter($elementIDs, function ($el) { return is_numeric($el);});
        return $this;
    }
