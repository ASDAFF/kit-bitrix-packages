...
if (isset($arResult['ITEM']))
{
    ...
    ?>
	<div id="<?=$this->GetEditAreaID($item['ID'])?>" class="check_basket_<?=$item['ID']?> product_card__block_item_inner-wrapper swiper-slide">
		<div class="product_card__block_item_inner" id="<?=$areaId?>" data-entity="item">
            <?
            $documentRoot = Main\Application::getDocumentRoot();
            // ----------------------------------------------------------------------------------------------------------------------------------
             $templatePath = strtolower($arResult['TYPE']).'/'.(isset($arResult['TEMPLATE'])?$arResult['TEMPLATE']:'template_1').'.php';
