...
function bcNav(page, th)
{
	...
	setTimeout(function() {
		for(i=1; i <= <?=$arResult["PAGE_COUNT"]?>; i++)
		{
			if(i == page)
			{
				BX.addClass(BX('blog-comment-nav-t'+i).parentNode, 'active');
				BX.addClass(BX('blog-comment-nav-b'+i).parentNode , 'active');
				BX('blog-comment-page-'+i).style.display = "";
			}
			else
			{
				BX.removeClass(BX('blog-comment-nav-t'+i).parentNode, 'active');
				BX.removeClass(BX('blog-comment-nav-b'+i).parentNode, 'active');
