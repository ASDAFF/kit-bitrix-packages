	...
	var repo = {},
		...
		d.prototype = {
			...
			onAttachFiles : function(files) {
				...
				if (files && this.inited === true && this.params['maxCount'] > 0)
				{
					if (this.params['maxCount'] == 1 && files.length > 0)
					{
						...
						{
							this._deleteFile(this.agent.getItems().getFirst(), false);
						}
			...
			deleteFile : function(item) {
				...
				var newInput;
				if (0 && item.file["justUploaded"] === true && item.file["file_id"] > 0)
				{
					var data = {
			...
			_deleteFile : function(item) { // for compatibility. Do not Use
				_this = this;
				...
				$.ajax({
					...
					data: {
						...
						"code": _this.agent.params.code,
						"action": _this.agent.params.action,
						"value": 0,
						"part": _this.agent.params.part,
						"site": _this.agent.params.site,
						"siteTemplate": _this.agent.params.siteTemplate,
						"page": _this.agent.params.page
					},
					success: function(data) {
						if($(".block-wrapper[data-id="+_this.agent.params.id+"]").length)
						{
							$(".block-wrapper[data-id="+_this.agent.params.id+"]").after(data);
							$(".block-wrapper[data-id="+_this.agent.params.id+"]:first").remove();
						}
						else
						{
							if($(".block-wrapper[data-copyof="+_this.agent.params.id+"]").length)
							{
								$(".block-wrapper[data-copyof="+_this.agent.params.id+"]").after(data);
								$(".block-wrapper[data-copyof="+_this.agent.params.id+"]:first").remove();
							}
						}
