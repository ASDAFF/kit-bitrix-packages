...
foreach($arResult["CATEGORIES"] as $category_id => $arCategory)
{
	foreach($arCategory["ITEMS"] as $i => $arItem)
	{
		...
		{
            if($arResult['query']){
                $l = strlen($arResult['query']);
            }
            $arItem['NAME'] = '<span>'.substr($arItem['NAME'],0,$l).'</span>'.substr($arItem['NAME'],$l,strlen($arItem['NAME']) - $l);
...
if($arResult["CATEGORIES"]){
    $sku = \CCatalogSKU::GetInfoByProductIBlock(Option::get('IBLOCK_ID'));

    if($sku['IBLOCK_ID'] > 0){
        foreach($arResult["CATEGORIES"] as $category_id => $arCategory){
            foreach($arCategory["ITEMS"] as $i => $arItem){
                if($arItem['PARAM2'] == $sku['IBLOCK_ID']){
