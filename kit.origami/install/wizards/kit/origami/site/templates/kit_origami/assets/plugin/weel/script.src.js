...
const WheelOfFortune = function (wheelSelector, params) {
...
WheelOfFortune.prototype.getCenterWeel = function () {
    let weelClientRect = this.weel.getBoundingClientRect();
    let centrWeel = {
...
WheelOfFortune.prototype.getAngle = function (evt) {
    ...
    let coords = this.defineCoordsEvent(evt);
    let radians = Math.atan2(coords.x, coords.y);
...
WheelOfFortune.prototype.triggerItems = function (evt){
    ...
    if (this.numItemStart !== this.numItemFinish) {
        ...
        for(let i = 0; i < this.QUANTITY_ITEMS; i++) {
            let s = (this.start + i) % this.items.length;
...
WheelOfFortune.prototype.clonedItems = function() {
    if (this._cloneItems.length < this.QUANTITY_ITEMS) {
        let itemsList = this.weel.querySelector('ul');
...
WheelOfFortune.prototype.init = function(evt) {
    if (!this.isInit) {
        ...
        this.weel.classList.add('hide');
        this.itemsList.classList.add('nav-round-menu__wrapper');
        // this.centrWeel = this.getCenterWeel();
        ...
        this.delta = this.items.length - this.QUANTITY_ITEMS / 2;
        for(let i = 0; i < this.QUANTITY_ITEMS; i++) {
            let index = (this.delta + i) % this.items.length;
...
WheelOfFortune.prototype.destroy = function(evt) {
    if(this.isInit) {
        ...
        let cloneItems = Array.prototype.slice.call(this.itemsList.querySelectorAll('.clone'));
