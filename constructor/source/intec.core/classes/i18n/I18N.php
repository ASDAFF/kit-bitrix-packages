 ...
 * You can access that instance via `Core::$app->i18n`.
 *
...
class I18N extends Component
{
    ...
    {
        parent::init();
        if (!isset($this->translations['core']) && !isset($this->translations['core*'])) {
            $this->translations['core'] = [
                'class' => 'intec\core\i18n\PhpMessageSource',
                'sourceLanguage' => 'en-US',
                'basePath' => '@intec/core/messages',
            ];
        }
    }
    ...
    public function getMessageFormatter()
    {
        if ($this->_messageFormatter === null) {
            ...
            $this->_messageFormatter = Core::createObject($this->_messageFormatter);
        }
     ...
     * Can be given as array or string configuration that will be given to [[Core::createObject]] to create an instance
    ...
    public function getMessageSource($category)
    {
        if (isset($this->translations[$category])) {
            ...
            if ($source instanceof MessageSource) {
                ...
                return $this->translations[$category] = Core::createObject($source);
            }
        } else {
            ...
            foreach ($this->translations as $pattern => $source) {
                if (strpos($pattern, '*') > 0 && strpos($category, rtrim($pattern, '*')) === 0) {
                    if ($source instanceof MessageSource) {
                        ...
                        return $this->translations[$category] = $this->translations[$pattern] = Core::createObject($source);
                    }
                }
            }
            // match '*' in the last
            if (isset($this->translations['*'])) {
                ...
                if ($source instanceof MessageSource) {
                    ...
                    return $this->translations[$category] = $this->translations['*'] = Core::createObject($source);
                }
            }
        }
