...
{
    /**
     * Номер последнего сгенерированного идентификатора на странице.
     * @var integer
     */
    protected static $identifier = 0;

    /**
     * Возвращает номер последнего сгенерированного идентификатора на странице.
     * @return integer
     */
    ...
    /**
     * Устанавливает номер последнего сгенерированного идентификатора на странице.
     * @param integer $identifier
     */
    public static function setIdentifier($identifier)
    {
        $identifier = Type::toInteger($identifier);
    ...
    /**
     * Возвращает валидный идентификатор.
     * @param string $id Строка-идентификатор
     * @param string|null $prefix Дополнительный префикс.
     */
     ...
     * @param null $prefix Префикс.
     * @param null $addition Дополнение.
     * @return string
     */
     ...
     * @param string $name the name attribute.
     * @param string|array $value the value attribute. If it is null, the value attribute will not be generated.
     * @param array $options the tag options in terms of name-value pairs. These will be rendered as
     * the attributes of the resulting tag. The values will be HTML-encoded using [[encode()]].
     * If a value is null, the corresponding attribute will not be rendered.
     * See [[renderTagAttributes()]] for details on how attributes are being rendered.
    ...
    public static function hiddenInput($name, $value = null, $options = [])
    {
        if (Type::isArrayable($value)) {
            $function = function ($value, $keys = null) use (&$function, &$name, &$options) {
                $html = '';

                if (Type::isArrayable($value)) {
                    foreach ($value as $key => $item)
                        $html .= $function($item, $keys.'['.$key.']');
                } else {
                    $html .= static::input('hidden', $name.$keys, $value, $options);
                }
        ...
        foreach ($attributes as $name => $value) {
            if (is_bool($value)) {
                ...
                if (in_array($name, static::$dataAttributes)) {
                    ...
                    if (Type::isArrayable($value))
                        $value = static::cssClassFromArray($value);
    ...
    /**
     * Обрабатывает массив классов и преобразует их в строку.
     * @param array $class Массив классов.
     * @param bool $handleKeys Обрабатывать ключи массива как классы, а значение - как логику.
     * @return string
     * @since 1.0.2
     */
    public static function cssClassFromArray(array $class, $handleKeys = false)
    {
        if (!Type::isArrayable($class))
            return null;

        $result = [];
        $function = function ($array, $parent = null) use (&$function, &$result, &$handleKeys) {
            if ($handleKeys) {
                foreach ($array as $name => $condition) {
                    if (
                        empty($name) && !Type::isNumeric($name) &&
                        empty($parent) && !Type::isNumeric($parent)
                    ) continue;

                    if (!empty($parent) || Type::isNumeric($parent))
                        $name = $parent.((!empty($name) || Type::isNumeric($name)) ? '-'.$name : null);

                    if (Type::isArrayable($condition)) {
                ...
                foreach ($array as $key => $name) {
                    if (Type::isArrayable($name)) {
                        if (!empty($key) || Type::isNumeric($key)) {
                            if (!empty($parent) || Type::isNumeric($parent))
                                $key = $parent.'-'.$key;
    ...
    /**
     * Обрабатывает строку классов и преобразует их в массив.
     * @param string $class Строка классов.
     * @return array
     * @since 1.0.2
     */
    public static function cssClassToArray($class)
    {
        $class = Type::toString($class);
        $result = [];
    ...
    /**
     * Режим минификации: Пробелы и переносы строк.
     */
    const MINIMIZE_SPACES = 1 << 0;
    /**
     * Режим минификации: Тэги <pre> и некоторые необязательные атрибуты тегов.
     */
    const MINIMIZE_TAGS = 1 << 1;
    /**
     * Режим минификации: Комментарии.
     */
    const MINIMIZE_COMMENTARIES = 1 << 2;
    /**
     * Режим минификации: Лишние пробелы и переносы строк в контенте.
     */
    const MINIMIZE_CONTENT = 1 << 3;
    /**
     * Режим минификации: Все.
     */
    const MINIMIZE_ALL = self::MINIMIZE_SPACES | self::MINIMIZE_TAGS | self::MINIMIZE_COMMENTARIES | self::MINIMIZE_CONTENT;

    /**
     * Минимизирует HTML код.
     * @param string $value Html код для минимизации.
     * @param integer $mode Режим работы минимизатора.
     * @return string
     */
    public static function minimize($value, $mode = self::MINIMIZE_ALL)
    {
        if ($mode & self::MINIMIZE_COMMENTARIES) {
            $value = RegExp::replaceBy('/(<!--(.*?)-->)/is', '', $value);
        }

        if ($mode & self::MINIMIZE_TAGS) {
            $value = RegExp::replaceBy('/type=[\'"]text\\/(javascript|css)[\'"]/', '', $value);
            $value = RegExp::replaceBy('/(<pre>(.*?)<\\/pre>)/is', '', $value);
        }

        if ($mode & self::MINIMIZE_SPACES) {
            $value = RegExp::replaceBy('/\\t/', ' ', $value);
            $value = RegExp::replaceBy('/(^[\\n]*|[\\n]+)[\\s]*[\\n]+/', "\n", $value);
            $value = RegExp::replaceBy('/>\\s+</', '><', $value);
            $value = RegExp::replaceBy('/^[[:blank:]]+/m', '', $value);
            $value = RegExp::replaceBy('/[[:blank:]]+/', ' ', $value);
        }

        if ($mode & self::MINIMIZE_CONTENT) {
            $value = RegExp::replaceBy('/>[^\\S ]+/s', '>', $value);
            $value = RegExp::replaceBy('/[^\\S ]+</s', '<', $value);
            $value = RegExp::replaceBy('/(\\s)+/s', '${0}', $value);
        }
    ...
    /**
     * Обрезает Html-теги, кроме разрешенных
     * @param string $string Строка, из которой удаляются теги.
     * @param array|string $tags Разрешенные теги.
     * @return string
     */
    public static function stripTags($string, $tags = null) {
