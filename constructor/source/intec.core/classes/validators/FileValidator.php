...
class FileValidator extends Validator
{
    ...
    public function init()
    {
        ...
        if ($this->message === null) {
            $this->message = Core::t('core', 'File upload failed.');
        }
        if ($this->uploadRequired === null) {
            $this->uploadRequired = Core::t('core', 'Please upload a file.');
        }
        if ($this->tooMany === null) {
            $this->tooMany = Core::t('core', 'You can upload at most {limit, number} {limit, plural, one{file} other{files}}.');
        }
        if ($this->wrongExtension === null) {
            $this->wrongExtension = Core::t('core', 'Only files with these extensions are allowed: {extensions}.');
        }
        if ($this->tooBig === null) {
            $this->tooBig = Core::t('core', 'The file "{file}" is too big. Its size cannot exceed {formattedLimit}.');
        }
        if ($this->tooSmall === null) {
            $this->tooSmall = Core::t('core', 'The file "{file}" is too small. Its size cannot be smaller than {formattedLimit}.');
        }
        ...
        if ($this->wrongMimeType === null) {
            $this->wrongMimeType = Core::t('core', 'Only files with these MIME types are allowed: {mimeTypes}.');
        }
                ...
                if ($this->maxSize !== null && $value->size > $this->getSizeLimit()) {
                    return [
                        ...
                        [
                            ...
                            'formattedLimit' => Core::$app->formatter->asShortSize($this->getSizeLimit()),
                        ],
                    ];
                    ...
                    return [
                        ...
                        [
                            'file' => $value->name,
                            'limit' => $this->minSize,
                            'formattedLimit' => Core::$app->formatter->asShortSize($this->minSize),
                        ],
                    ];
     ...
     * @throws \intec\core\base\InvalidConfigException
     * @see mimeTypes
     * @since 2.0.8
     */
