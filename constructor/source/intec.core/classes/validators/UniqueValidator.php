...
use intec\core\db\ActiveQueryInterface;
    ...
    public function init()
    {
        ...
        if (is_array($this->targetAttribute) && count($this->targetAttribute) > 1) {
            ...
            if ($this->comboNotUnique === null) {
                $this->message = Core::t('core', 'The combination {values} of {attributes} has already been taken.');
            } else {
                $this->message = $this->comboNotUnique;
            }
        } else {
            $this->message = Core::t('core', '{attribute} "{value}" has already been taken.');
        }
    }
