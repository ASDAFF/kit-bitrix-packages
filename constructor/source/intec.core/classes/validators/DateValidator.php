...
class DateValidator extends Validator
{
    ...
    public function init()
    {
        ...
        if ($this->message === null) {
            $this->message = Core::t('core', 'The format of {attribute} is invalid.');
        }
        if ($this->format === null) {
            if ($this->type === self::TYPE_DATE) {
                $this->format = Core::$app->formatter->dateFormat;
            } elseif ($this->type === self::TYPE_DATETIME) {
                $this->format = Core::$app->formatter->datetimeFormat;
            } elseif ($this->type === self::TYPE_TIME) {
                $this->format = Core::$app->formatter->timeFormat;
            } else {
        ...
        if ($this->min !== null && $this->tooSmall === null) {
            $this->tooSmall = Core::t('core', '{attribute} must be no less than {min}.');
        }
        if ($this->max !== null && $this->tooBig === null) {
            $this->tooBig = Core::t('core', '{attribute} must be no greater than {max}.');
        }
    ...
    private function parseDateValueIntl($value, $format)
    {
        ...
        if ($parsedDate === false || $parsePos !== mb_strlen($value, Core::$app ? Core::$app->charset : 'UTF-8')) {
            return false;
        }
