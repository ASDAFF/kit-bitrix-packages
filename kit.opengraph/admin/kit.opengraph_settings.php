...
if((isset($_REQUEST['save']) && !empty($_REQUEST['save']) || (isset($_REQUEST['apply']) && !empty($_REQUEST['apply'])))) {
    ...
    $save = true;
    $arFields = array_filter(array_keys(\Kit\Opengraph\OpengraphPageMetaTable::getmap()), function($val) {
        return $val != 'ID';
    });
    ...
    $metaTable = new \Kit\Opengraph\OpengraphPageMetaTable();
    $checkRequeredFields = $metaTable->checkRuleToSave($arFields);
    
    if($checkRequeredFields) {
        Bitrix\Main\Config\Option::set(KitOpengraph::MODULE_ID, 'OPENGRAPH_SETTINGS', serialize($arFields), $arFields['SITE_ID']);
    } else {
        ...
        <td colspan="2" style="text-align:  center;">
            <div class="adm-info-message-wrap">
                <div class="adm-info-message">
                    <?=Loc::getMessage("KIT_OPENGRAPH_EXCEPTION_NOTE");?>
                </div>
            </div>
        </td>
    </tr>
