...
class sotbit_regions extends CModule
{
    ...
    function InstallEvents()
    {
        ...
        while ($_598634165 = $_870088534->fetch()) {
            $this->SotbitRegionsInstallData($_598634165['LID']);
            $this->SotbitRegionsSetSettings($_598634165['LID']);
        }
        return true;
    }
    ...
    function DoInstall()
    {
        ...
        if ($_REQUEST['step'] == 1) {
            if ($_SERVER['SERVER_NAME']) {
                $_598634165 = $_SERVER['SERVER_NAME'];
            } elseif ($_SERVER['HTTP_HOST']) {
                $_598634165 = $_SERVER['HTTP_HOST'];
            }
            $_846401276 = '';
            $_997912636 = \CUpdateClient::GetUpdatesList($_846401276);
            $_1913687933 = array('ACTION' => 'ADD', 'SITE' => $_598634165, 'KEY' => md5('BITRIX' . \CUpdateClientPartner::GetLicenseKey() . 'LICENCE'), 'LICENSE' => $_997912636['CLIENT'][0]['@']['LICENSE'], 'MODULE' => self::MODULE_ID, 'NAME' => $_REQUEST['Name'], 'EMAIL' => $_REQUEST['Email'], 'PHONE' => $_REQUEST['Phone'], 'BITRIX_DATE_FROM' => $_997912636['CLIENT'][0]['@']['DATE_FROM'], 'BITRIX_DATE_TO' => $_997912636['CLIENT'][0]['@']['DATE_TO'],);
            $_1654425955 = array('http' => array('method' => 'POST', 'header' => 'Content-Type: application/json; charset=utf-8', 'content' => json_encode($_1913687933)));
            $_524180966 = stream_context_create($_1654425955);
            $_956421550 = file_get_contents('https://www.sotbit.ru:443/api/datacollection/index.php', 0, $_524180966);
    ...
    function unInstallData($_1117533846)
    {
        ...
        $_870088534 = $_872752286->GetList(array(), array('ENTITY_ID' => 'SOTBIT_REGIONS'));
    ...
    function SotbitRegionsInstallData($_1117533846 = '')
    {
        $this->SotbitRegionsInstallProperties($_1117533846);
        $this->SotbitRegionsInstallDomains($_1117533846);
        $this->SotbitRegionsInstallFavorites($_1117533846);
        return true;
    }

    function SotbitRegionsInstallProperties($_1117533846)
    {
        $_83672819 = array();
        ...
        $_872752286 = new \CUserTypeEntity;
        $_185124254 = array('ENTITY_ID' => 'SOTBIT_REGIONS', 'SORT' => 100, 'MULTIPLE' => 'N', 'MANDATORY' => 'N', 'SHOW_FILTER' => 'N', 'SHOW_IN_LIST' => 'Y', 'EDIT_IN_LIST' => 'Y', 'IS_SEARCHABLE' => 'N', 'SETTINGS' => array(), 'EDIT_FORM_LABEL' => array(), 'LIST_COLUMN_LABEL' => array(), 'LIST_FILTER_LABEL' => array(), 'ERROR_MESSAGE' => array(), 'HELP_MESSAGE' => array(),);
        ...
        foreach ($_1293476724 as $_283758071 => $_753604381) {
            ...
            foreach ($_83672819 as $_1126291632) {
                $_1470270628['EDIT_FORM_LABEL'][$_1126291632] = Loc::getMessage('sotbit.regions_PROP_' . $_283758071);
                $_1470270628['LIST_COLUMN_LABEL'][$_1126291632] = Loc::getMessage('sotbit.regions_PROP_' . $_283758071);
                $_1470270628['LIST_FILTER_LABEL'][$_1126291632] = Loc::getMessage('sotbit.regions_PROP_' . $_283758071);
                $_1470270628['ERROR_MESSAGE'][$_1126291632] = Loc::getMessage('sotbit.regions_PROP_' . $_283758071);
                $_1470270628['HELP_MESSAGE'][$_1126291632] = Loc::getMessage('sotbit.regions_PROP_' . $_283758071);
            }
            $_1556584304 = $_872752286->Add($_1470270628);
        }
    }

    function SotbitRegionsInstallDomains($_1117533846)
    {
        $_2032066276 = array();
        $_91637380 = array();
        ...
        foreach ($_1152271029 as $_985790079 => $_511175772) {
            ...
            $_1473844007 = array('CODE' => $_479394219, 'NAME' => Loc::getMessage('sotbit.regions_DOMEN_' . $_511175772), 'SORT' => 100, 'PRICE_CODE' => $_2032066276, 'STORE' => $_91637380, 'SITE_ID' => [$_1117533846]);
            if ($_985790079 == 0) $_1166177176 = "'Y'"; else $_1166177176 = 'NULL';
            $DB->Query("INSERT INTO `sotbit_regions` VALUES (NULL,'" . $_1473844007["CODE"] . "', '" . $_1473844007["NAME"] . "', 100, '" . serialize($_1473844007["SITE_ID"]) . "', '" . serialize($_1473844007["PRICE_CODE"]) . "', '" . serialize($_1473844007["STORE"]) . "', NULL, NULL, NULL, NULL, NULL, NULL, " . $_1166177176 . ");");
            ...
            if ($_869891527 > 0) {
                ...
                $DB->Query("INSERT INTO `sotbit_regions_fields` VALUES (NULL," . $_869891527 . ", 'UF_PHONE', '" . serialize($_167336515) . "');");
                $DB->Query("INSERT INTO `sotbit_regions_fields` VALUES (NULL," . $_869891527 . ", 'UF_ADDRESS', '" . Loc::getMessage("sotbit.regions_ADDRESS", array("#HOME#" => rand(1, 50))) . "');");
                $DB->Query("INSERT INTO `sotbit_regions_fields` VALUES (NULL," . $_869891527 . ", 'UF_EMAIL', '" . serialize(["sales@" . $_1106395561 . $_1395317437]) . "');");
                $DB->Query("INSERT INTO `sotbit_regions_fields` VALUES (NULL," . $_869891527 . ", 'UF_ROBOTS', '');");
                if (Loader::includeModule('sale')) {
                    $_989329412 = \Bitrix\Sale\Location\LocationTable::getList(['filter' => ['=NAME.NAME' => Loc::getMessage('sotbit.regions_LOCATION_' . $_511175772), '=NAME.LANGUAGE_ID' => LANGUAGE_ID,], 'select' => ['*', 'NAME.*',], 'cache' => ['ttl' => 36000000,],])->fetch();
                    if ($_989329412['ID'] > 0) {
                        $DB->Query("INSERT INTO `sotbit_regions_locations` VALUES (NULL," . $_869891527 . "," . $_989329412["ID"] . ");");
                    }
                    if ($_511175772 == '') {
                        $_989329412 = \Bitrix\Sale\Location\LocationTable::getList(['filter' => ['=NAME.NAME' => Loc::getMessage('sotbit.regions_LOCATION_MOSCOW'), '=NAME.LANGUAGE_ID' => LANGUAGE_ID,], 'select' => ['*', 'NAME.*',], 'cache' => ['ttl' => 36000000,],])->fetch();
                    }
                    if ($_989329412['ID'] > 0) {
                        $DB->Query("INSERT INTO `sotbit_regions_locations` VALUES (NULL," . $_869891527 . "," . $_989329412["ID"] . ");");
                    }
    ...
    function SotbitRegionsSetSettings($_1117533846)
    {
        global $DB;
        $DB->Query("INSERT INTO `sotbit_regions_options` VALUES ('SINGLE_DOMAIN','Y', '" . $_1117533846 . "');");
        if (Loader::includeModule("statistic")) {
            $DB->Query("INSERT INTO `sotbit_regions_options` VALUES ('FIND_USER_METHOD','statistic', '" . $_1117533846 . "');");
        } elseif (function_exists("curl_version")) {
            $DB->Query("INSERT INTO `sotbit_regions_options` VALUES ('FIND_USER_METHOD','ipgeobase', '" . $_1117533846 . "');");
        } else {
            $DB->Query("INSERT INTO `sotbit_regions_options` VALUES ('FIND_USER_METHOD','geoip','" . $_1117533846 . "');");
        }
        $DB->Query("INSERT INTO `sotbit_regions_options` VALUES ('INSERT_SALE_LOCATION','N', '" . $_1117533846 . "');");
        $DB->Query("INSERT INTO `sotbit_regions_options` VALUES ('MULTIPLE_DELIMITER',', ', '" . $_1117533846 . "');");
        $_1389722182 = 5;
        if (Loader::includeModule("sale")) {
            $_2069164710 = TypeTable::getList(["select" => ["ID"], "filter" => ["CODE" => "CITY"]])->fetch();
            if (!empty($_2069164710["ID"])) $_1389722182 = $_2069164710["ID"];
        }
        $DB->Query("INSERT INTO `sotbit_regions_options` VALUES ('LOCATION_TYPE', '" . $_1389722182 . "', '" . $_1117533846 . "');");
    }

    function SotbitRegionsInstallFavorites($_1117533846)
    {
        if (Loader::includeModule('sale')) {
            ...
            foreach ($_748595033 as $_1452412573) {
                $_989329412 = \Bitrix\Sale\Location\LocationTable::getList(['filter' => ['=NAME.NAME' => Loc::getMessage('sotbit.regions_FAVORITE_' . $_1452412573), '=NAME.LANGUAGE_ID' => LANGUAGE_ID,], 'select' => ['*', 'NAME.*',], 'cache' => ['ttl' => 36000000,],])->fetch();
                if ($_989329412['CODE']) {
    ...
    function SotbitRegionsGetAllFiles($_1358015652)
    {
    }

    function SotbitRegionsDeleteFiles($_543546171)
    {
    }
} ?>