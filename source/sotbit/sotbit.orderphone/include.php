...
Class CSotbitOrderphone
{
	...
	function CSotbitOrderphone($arParams, $fields)
	{
	...
	function StartAjax()
	{
		...
		if (!$USER->IsAuthorized())
		{
			...
			{
				COption::RemoveOption("sotbit.orderphone", "SOTBIT_ORDER_USER");
				$filter = Array
				(
						"LOGIN" => $email."%",
						);
			...
			{
				$this->userID = COption::GetOptionString("sotbit.orderphone", "SOTBIT_ORDER_USER");
				if(!$this->userID)
				{
					$this->AddUser();
					COption::SetOptionString("sotbit.orderphone", "SOTBIT_ORDER_USER", $this->userID);
