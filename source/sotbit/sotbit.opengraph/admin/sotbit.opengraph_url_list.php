...
use Sotbit\Opengraph\Helper\OpengraphMetaAction;
...
$rsData->NavStart();
$lAdmin->SetNavigationParams($rsData, array("BASE_LINK" => '/bitrix/admin/sotbit.opengraph_url_list.php'));
...
while($arRes = $rsData->NavNext(false)) {
    ...
    if($itemType == "S") {
        ...
        $url_section = '/bitrix/admin/sotbit.opengraph_url_list.php?'.implode('&', $arLink);
    } else {
        ...
        $url_element = '/bitrix/admin/sotbit.opengraph_url_edit.php?'.implode('&', $arLink);
    }
    ...
    if($itemType == "S")
        $row->AddViewField($itemType."ID", '<a href="/bitrix/admin/sotbit.opengraph_category_edit.php?lang=ru&site='.$_REQUEST['site'].'&ID='.$arRes['ID'].(isset($_REQUEST['CATEGORY_ID']) ? '&CATEGORY_ID='.$_REQUEST['category'] : '').'" title="'.GetMessage("IBLIST_A_EDIT").'">'.$itemId.'</a>');
    else
        $row->AddViewField("ID", '<a href="/bitrix/admin/sotbit.opengraph_url_edit.php?lang=ru&site='.$_REQUEST['site'].'&ID='.$arRes['ID'].(isset($_REQUEST['CATEGORY_ID']) ? '&CATEGORY_ID='.$_REQUEST['category'] : '').'" title="'.GetMessage("IBLIST_A_EDIT").'">'.$itemId.'</a>');
    ...
    if($APPLICATION->GetGroupRight('sotbit.opengraph') == "W") {
        ...
        $arActions[] = array(
            ...
            "ACTION" => $lAdmin->ActionRedirect('/bitrix/admin/sotbit.opengraph_'.(($itemType == 'S') ? 'category' : 'url').'_edit.php?lang=ru&site='.$_REQUEST['site'].'&ID='.$arRes['ID'].(isset($_REQUEST['CATEGORY_ID']) ? '&CATEGORY_ID='.$_REQUEST['CATEGORY_ID'] : '').''),
            "DEFAULT" => true,
        );
        $arActions[] = array(
            "ICON" => "delete",
...
$aContext[] = array(
    ...
    "LINK" => '/bitrix/admin/sotbit.opengraph_url_edit.php?'.implode('&', $arrLink)
);
$aContext[] = array(
    "TEXT" => Loc::getMessage($langPrefix.'CREATE_CATEGORY'),
    "ICON" => "btn_new",
    "LINK" => '/bitrix/admin/sotbit.opengraph_category_edit.php?'.implode('&', $arrLink)
);
//$lAdmin->setContextSettings(array("pagePath" => '/bitrix/admin/sotbit.opengraph_url_list.php'));
if($APPLICATION->GetGroupRight('sotbit.opengraph') == "W")
