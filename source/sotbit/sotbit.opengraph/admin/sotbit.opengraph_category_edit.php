...
use Sotbit\Opengraph\Helper\OpengraphHelper;
...
if((isset($_REQUEST['save']) && !empty($_REQUEST['save']) || (isset($_REQUEST['apply']) && !empty($_REQUEST['apply'])))) {
    ...
    if($result->isSuccess()) {
        if(isset($_REQUEST['save']) && !empty($_REQUEST['save'])) {
            LocalRedirect('/bitrix/admin/sotbit.opengraph_url_list.php?lang='.SITE_ID.'&site='.$_REQUEST['site']);
        }
        else {
...
$aMenu= array(array(
                  "TEXT" => GetMessage( "SOTBIT_OPENGRAPH_EDIT_BACK" ),
                  "TITLE" => GetMessage( "SOTBIT_OPENGRAPH_EDIT_BACK_TITLE" ),
                  "ICON" => "btn_list",
                  'MENU' =>array(
                      array(
                          "TEXT" => GetMessage( "SOTBIT_OPENGRAPH_EDIT_LIST" ),
                          "TITLE" => GetMessage( "SOTBIT_OPENGRAPH_EDIT_LIST_TITLE" ),
                          "LINK" => '/bitrix/admin/sotbit.opengraph_url_list.php?lang='.SITE_ID.(isset($_REQUEST['site']) ? '&site='.$_REQUEST['site'] : ''),
                          "ICON" => "btn_list"
                      ),
                      array(
                          "TEXT" => GetMessage( "SOTBIT_OPENGRAPH_EDIT_SECTION_BACK" ),
                          "TITLE" => GetMessage( "SOTBIT_OPENGRAPH_EDIT_SECTION_BACK_TITLE" ),
                          "LINK" => '/bitrix/admin/sotbit.opengraph_url_list.php?lang='.SITE_ID.(isset($_REQUEST['site']) ? '&site='.$_REQUEST['site'] : '').(isset($_REQUEST['category']) ? '&category='.$_REQUEST['category'] : ''),
                      )
                  )),
);
