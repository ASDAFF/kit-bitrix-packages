...
if((isset($_REQUEST['save']) && !empty($_REQUEST['save']) || (isset($_REQUEST['apply']) && !empty($_REQUEST['apply'])))) {
    ...
    if($result->isSuccess()) {
        if(isset($_REQUEST['save']) && !empty($_REQUEST['save'])) {
            LocalRedirect('/bitrix/admin/sotbit_crosssell_list.php?lang='.SITE_ID.'&site='.$_REQUEST['site']);
        }
        else {
...
if(isset($_SESSION['SAVE_CROSSSELL'][intval($_REQUEST['ID'])])) {
    ...
    CAdminMessage::ShowMessage(array(
        "MESSAGE" => Loc::getMessage("SOTBIT_CROSSSELL_SUCCESS_SAVE"),
        "TYPE" => "OK"
    ));
}
...
$aTabs = array(
    array(
        "DIV" => "general_tab",
        "TAB" => Loc::getMessage("SOTBIT_CROSSSELL_GENERAL_EDIT"),
        "ICON" => "main_user_edit",
        "TITLE" => Loc::getMessage("SOTBIT_CROSSSELL_GENERAL_TITLE")
    ),
);
...
if($APPLICATION->GetGroupRight('sotbit.crosssell') == "W")
{
    ...
    if($_REQUEST['ID'])
    {
        $urlDelete = '/bitrix/admin/sotbit_crosssell_list.php?lang'.SITE_ID.(isset($_REQUEST['site']) ? '&site='.$_REQUEST['site'] : '').'&sessid='.bitrix_sessid().'&type=S&delete='.$_REQUEST['ID'];

        $aMenu[] = array(
            "TEXT" => GetMessage( "SOTBIT_CROSSSELL_DELETE_BLOCK"),
            "ONCLICK" => "javascript:if(confirm('".GetMessageJS("SOTBIT_CROSSSELL_CATEGORY_DELETE_BLOCK_TEXT")."'))top.window.location.href='".CUtil::JSEscape($urlDelete)."'",
            "ICON" => "btn_delete",
        );
    }
}
...
if($event->getResults()) {
    ...
    if (is_array($handlerResult['template_field'])){
        ...
        $tabControl->BeginCustomField( "ITEM_TEMPLATE", Loc::getMessage('SOTBIT_CROSSSELL_TAB_1_TEMPLATE'), false );
        ?>
        <tr id="ITEM_TEMPLATE">
    ...
    if (is_array($handlerResult['show_slider_field'])){
        ...
        $tabControl->BeginCustomField( "USE_SLIDER", Loc::getMessage('SOTBIT_CROSSSELL_TAB_1_SLIDER'), false );
        ?>
        <tr id="USE_SLIDER">
            <td width="40%"><? echo $tabControl->GetCustomLabelHTML(); ?></td>
            <td width="60%">
    ...
    $tabControl->AddEditField("PRODUCT_NUMBER", Loc::GetMessage('SOTBIT_CROSSSELL_TAB_1_PRODUCT_NUMBER'), false, false, (isset($extraSettings['PRODUCT_NUMBER'])) ? $extraSettings['PRODUCT_NUMBER']:'10');
}
