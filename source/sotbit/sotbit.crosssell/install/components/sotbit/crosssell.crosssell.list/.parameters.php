...
$isOrigami = Loader::includeModule('sotbit.origami');
...
$db_crosssell_category = \Sotbit\Crosssell\Orm\CrosssellCategoryTable::getList(array(
    'select' =>
        array(
            'ID', 'NAME'
        ),
...
$db_crosssell = \Sotbit\Crosssell\Orm\CrosssellTable::getList(array(
    'filter' =>
        array(
...
$arCrosssellCategory = array();

while ($collection_item = $db_crosssell_category->fetch()){
    $arCrosssellCategory[$collection_item['ID']] = $collection_item['NAME'];
}

$arCrosssell = array();

while ($item = $db_crosssell->fetch()){
    $arCrosssell[$item['CATEGORY_ID']][$item['ID']] = $item['NAME'];
}

$result = array();
foreach ($arCrosssell as $catId => $crosssells) {
    if ($arCrosssellCategory[$catId] == "") {
        $categoryName = GetMessage('ROOT_CATEGORY');
    }
    else
    {
        $categoryName = $arCrosssellCategory[$catId];
    }
    $result[$catId]['CATEGORY_NAME'] = $categoryName;
    $result[$catId]['ITEMS'] = array();
    if (is_array($crosssells)){
        foreach ($crosssells as $k => $v){
            $result[$catId]['ITEMS'][$k] = $v;
        }
    }
}

$arParametersCrosssell = array();
foreach ($result as $catId => $category) {
    $arParametersCrosssell["s" . $catId] = $category[CATEGORY_NAME];
    foreach ($category['ITEMS'] as $crosssellId => $crosssellName) {
        $arParametersCrosssell["e" . $crosssellId] = " . " . $crosssellName;
    }
}
...
$arShowTabs = array('Y' => GetMessage('IBLOCK_SHOW_TABS_TABS'), 'N' => GetMessage('IBLOCK_SHOW_TABS_ROWS'));
...
$sectionMode = array(
    "Y" => GetMessage("SECTION_MODE_Y"),
    "N" => GetMessage("SECTION_MODE_N")
);

$showSlider = array(
    "Y" => GetMessage("SHOW_SLIDER_Y"),
    "N" => GetMessage("SHOW_SLIDER_N")
);
...
$arComponentParameters = array(
    ...
    'PARAMETERS' => array(
        ...
        'SHOW_TABS' => array(
            'PARENT' => 'VISUAL',
            'NAME' => GetMessage('IBLOCK_SHOW_TABS'),
            'TYPE' => 'LIST',
            'DEFAULT' => 'Y',
            'VALUES' => $arShowTabs,
        ),
        'SHOW_SLIDER' => array(
            'PARENT' => 'VISUAL',
            'NAME' => GetMessage('SHOW_SLIDER'),
            'TYPE' => 'LIST',
            'DEFAULT' => 'N',
            'VALUES' => $showSlider,
            'HIDDEN' => $isOrigami ? "N" : "Y"
        ),
...
if ($arCurrentValues['SECTION_MODE'] === 'Y')
{
    $arComponentParameters['PARAMETERS']['CROSSSELL_LIST'] = array(
        "PARENT" => "BASE",
        "NAME" => GetMessage("CROSSSELL_LIST"),
        "TYPE" => "LIST",
        "VALUES" => $arParametersCrosssell,
        "MULTIPLE" => "Y",
        "REFRESH" => "N",
        "SIZE" => "9",
    );
    $arComponentParameters['PARAMETERS']['INTERRUPT_MODE'] = array(
        "PARENT" => "BASE",
        "NAME" => GetMessage("INTERRUPT_MODE"),
        'TYPE' => 'CHECKBOX',
        'DEFAULT' => 'Y',
    );
}
