        ...
        switch($action) {
            case 'delete' :
                foreach($idList as $id)  {
                    ...
                    if(strpos($id, "E") !== false) {
                        $entity = '\Sotbit\Crosssell\Orm\CrosssellTable';
                    } else {
                        $catId = str_replace(array('S'), array(''), $id);
                        $entity = '\Sotbit\Crosssell\Orm\CrosssellCategoryTable';
                        $rs = CrosssellTable::getList(array(
                            'select' => array('ID'),
            ...
            case 'edit':
                foreach($idList as $id => $arFields) {
                    ...
                    $id = str_replace(array('E', 'S'), array('',''), $id);
                    if($arFields['Active'] == Loc::getMessage('SOTBIT_CROSSSELL_ACTION_YES')) {
                        $arFields['Active'] = 'Y';
                    } else {
                        $arFields['Active'] = 'N';
                    }
