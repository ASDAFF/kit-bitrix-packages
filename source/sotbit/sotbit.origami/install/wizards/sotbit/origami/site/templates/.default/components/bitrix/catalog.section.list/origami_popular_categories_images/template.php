...
<div class="puzzle_block popular_category__wrapper main-container">
    ...
    <div class="popular_category_block">
        ...
        foreach($arResult["SECTIONS"] as $arSection)
        {
            ...
            if($i <= 3)
                $divClass = "col-xl-4 col-lg-4 col-md-4 col-sm-6 col-12 mb-3 mt-3 popular_category_block__canvas";
            else
                $divClass = "col-xl-3 col-lg-3 col-md-4 col-sm-6 col-12 mb-3 mt-3 popular_category_block__canvas";
            ?>

            <div class="<?=$divClass?>">
                <a class="popular_category_block__canvas_link <?=$hoverClass?>" title="<?=$arSection["NAME"]?>" href="<?=$arSection["SECTION_PAGE_URL"]?>" id="<?=$this->GetEditAreaId($arSection['ID']);?>">
                    <div class="popular_category_block__canvas_block_img">
                        <div class="popular_category_block__canvas_title fonts__small_title">
                            <?=$arSection["NAME"]?>
                        </div>
                        <img class="popular_category_block__canvas_img <?=$lazyClass?>"
                            <?=$strLazyLoad?>
