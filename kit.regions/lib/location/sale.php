...
class Sale
{
	...
	private function setPropertyId($personTypeId = 0)
	{
		...
		if(
			isset($_SESSION['KIT_REGIONS']['SALE_LOCATION'][$personTypeId]) &&
			is_array($_SESSION['KIT_REGIONS']['SALE_LOCATION'][$personTypeId]) > 0 &&
			key($_SESSION['KIT_REGIONS']['SALE_LOCATION'][$personTypeId]) > 0
		)
		{
			$propertyId = key($_SESSION['KIT_REGIONS']['SALE_LOCATION'][$personTypeId]);
		}
		else
		{
			...
			if($property['ID'] > 0)
			{
				...
				$_SESSION['KIT_REGIONS']['SALE_LOCATION'][$personTypeId] = array($property['ID'] => 0);
			}
		}
		return $propertyId;
	}
	...
	private function setId($personTypeId = 0)
	{
		try
		{
			...
			$return = 0;
			if($_SESSION['KIT_REGIONS']['SALE_LOCATION'][$personTypeId][self::$propertyId] > 0)
			{
				$return = $_SESSION['KIT_REGIONS']['SALE_LOCATION'][$personTypeId][self::$propertyId];
			}
			else
			{
				$domain = new Domain();
