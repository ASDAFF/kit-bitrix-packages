...
if ($_REQUEST['point_map_yandex_kit_regions_code_1__n0_lat']) {
    $mapY = [];
    $val = '';
    foreach ($_REQUEST as $k => $v) {
        if (strpos($k, 'point_map_yandex_kit_regions') !== false
            && strpos($k, 'lat') !== false
            && $v
        ) {
            $val = $v.',';
...
if ($_REQUEST['point_map_google_kit_regions_code2__n0_lat']) {
    $mapG = [];
    $val = '';
    foreach ($_REQUEST as $k => $v) {
        if (strpos($k, 'point_map_google_kit_regions') !== false
            && strpos($k, 'lat') !== false
            && $v
        ) {
            $val = $v.',';
...
if ($_REQUEST['ID'] > 0) {
    ...
    $APPLICATION->SetTitle(Loc::getMessage(\KitRegions::moduleId.'_NEW'));
}
...
$aTabs = [
    [
        'DIV'   => 'edit1',
        'TAB'   => Loc::getMessage(\KitRegions::moduleId.'_REGION'),
        'ICON'  => 'ad_contract_edit',
        'TITLE' => Loc::getMessage(\KitRegions::moduleId.'_REGION'),
    ],
    [
        'DIV'   => 'edit4',
        'TAB'   => Loc::getMessage(\KitRegions::moduleId.'_REGION_MAPS'),
        'ICON'  => 'ad_contract_edit',
        'TITLE' => Loc::getMessage(\KitRegions::moduleId.'_REGION_MAPS'),
    ],
    [
        'DIV'   => 'edit2',
        'TAB'   => Loc::getMessage(\KitRegions::moduleId.'_REGION_PRICE'),
        'ICON'  => 'ad_contract_edit',
        'TITLE' => Loc::getMessage(\KitRegions::moduleId.'_REGION_PRICE'),
    ],
    [
        'DIV'   => 'edit3',
        'TAB'   => Loc::getMessage(\KitRegions::moduleId.'_LOCATION_TAB'),
        'ICON'  => 'ad_contract_edit',
        'TITLE' => Loc::getMessage(\KitRegions::moduleId.'_LOCATION_TAB'),
    ],
];

$tabControl = new CAdminForm('kit_regions_edit', $aTabs);
...
if ($action == 'delete') {
    ...
    LocalRedirect('kit_regions.php?site='.$site.'&lang='.LANGUAGE_ID.'&site='.$_REQUEST['site']);
}
...
if ($action == 'copy' && $_REQUEST['ID'] > 0) {
    ...
    if ($id > 0) {
        ...
        LocalRedirect('kit_regions_edit.php?ID='.$id.'&lang='.LANGUAGE_ID.'&site='.$_REQUEST['site']);
    }
}
...
if ((strlen($save) > 0 || strlen($apply) > 0) && $REQUEST_METHOD == 'POST'
    ...
    if ($result->isSuccess()) {
        if (strlen($save) > 0) {
            LocalRedirect('kit_regions.php?lang='.LANGUAGE_ID.'&site='.$_REQUEST['site']);
        } else {
            LocalRedirect('kit_regions_edit.php?ID='.intval($ID).'&lang='
...
$aMenu = [
    [
        'TEXT'  => Loc::getMessage(\KitRegions::moduleId.'_RETURN'),
        'TITLE' => Loc::getMessage(\KitRegions::moduleId.'_RETURN'),
        'LINK'  => 'kit_regions.php?lang='.LANGUAGE_ID.'&site='.$_REQUEST['site'],
        'ICON'  => 'btn_list',
    ],
];
...
$aMenu[] = [
    'TEXT'  => Loc::getMessage(\KitRegions::moduleId.'_COPY'),
    'TITLE' => Loc::getMessage(\KitRegions::moduleId.'_COPY'),
...
$aMenu[] = [
    ...
    'TITLE' => Loc::getMessage(\KitRegions::moduleId.'_DELETE'),
...
$tabControl->BeginCustomField(
    "SITE_ID",
    Loc::getMessage(\KitRegions::moduleId.'_SITE_ID'),
    false
);
$sites = [];
    ...
    <td width="60%">
        <div class="adm-info-message-wrap">
            <div class="adm-info-message">
                <?= Loc::getMessage(\KitRegions::moduleId
                    .'_CODE_NOTE') ?>
            </div>
        </div>
    </td>
...
$tabControl->BeginCustomField("DEFAULT_DOMAIN",
    Loc::getMessage(\KitRegions::moduleId.'_DEFAULT_DOMAIN'),
    false
);
...
$tabControl->BeginCustomField(
    "COUNTER",
    Loc::getMessage(\KitRegions::moduleId.'_COUNTER'),
    false
);
?>
    <tr id="COUNTER">
...
$tabControl->BeginCustomField(
    "MANAGER",
    Loc::getMessage(\KitRegions::moduleId.'_MANAGER'),
    false
);
?>
    <tr id="MANAGER">
...
$tabControl->BeginCustomField(
    "MAP_YANDEX",
    Loc::getMessage(\KitRegions::moduleId.'_MAP_YANDEX'),
    false
);
?>
    <tr class="heading">
        <td><?= Loc::getMessage(\KitRegions::moduleId
    ...
    <tr id="MAP_YANDEX">
        <td>
            ...
            <?= Loc::getMessage(\KitRegions::moduleId.'_YANDEX_API_KEY') ?>
            <input type="text" name="YANDEX_API_KEY"
                   value="<?= $row["MAP_YANDEX"]['API_KEY'] ?>">
            <?= Loc::getMessage(\KitRegions::moduleId.'_REGION_MAPS_YANDEX_API') ?>
            <br>


            <?= Loc::getMessage(\KitRegions::moduleId.'_REGION_MAPS_MARKER') ?>
            <?$mapsMarker = new Kit\Regions\Config\Widgets\File(
                'YANDEX_MARKER',
                [
...
$tabControl->BeginCustomField(
    "MAP_GOOGLE",
    Loc::getMessage(\KitRegions::moduleId.'_MAP_GOOGLE'),
    false
);
?>
    <tr class="heading">
    ...
    <tr id="MAP_GOOGLE">
        <td>
            ...
            <?= Loc::getMessage(\KitRegions::moduleId.'_GOOGLE_API_KEY') ?>
            <input type="text" name="GOOGLE_API_KEY"
                   value="<?= $row["MAP_GOOGLE"]['API_KEY'] ?>">
            <?= Loc::getMessage(\KitRegions::moduleId.'_REGION_MAPS_GOOGLE_API') ?>
            <br>
...
$tabControl->BeginCustomField(
    "REGION_PRICE",
    Loc::getMessage(\KitRegions::moduleId.'_REGION_PRICE'),
    false
);
...
$arValueTypeSel["REFERENCE"] = [
    '-',
    Loc::getMessage(\KitRegions::moduleId."_PROCENT_UP"),
    Loc::getMessage(\KitRegions::moduleId."_PROCENT_DOWN"),
    Loc::getMessage(\KitRegions::moduleId."_FIX_UP"),
    Loc::getMessage(\KitRegions::moduleId."_FIX_DOWN"),
];
?>
    <tr class="heading">
        <td colspan="2"><?= Loc::getMessage(\KitRegions::moduleId
    ...
    <tr id="tr_VALUE" class="tabcontent ChangeValue">
        <td width="40%" class="left-side-wrapper bottom-left-side-wrapper">
            <div class="left-side bottom-left-side">
                <?= Loc::getMessage(\KitRegions::moduleId.'_PRICE_VALUE') ?>
            </div>
        </td>
...
if (Loader::includeModule('sale')) {
    ...
    $tabControl->BeginCustomField(
        "LOCATION",
        Loc::getMessage(\KitRegions::moduleId.'_LOCATION_ID'),
        false
    );
    ...
    <tr id="tr_VALUE" class="tabcontent LocationValue">
        <td width="40%" class="left-side-wrapper bottom-left-side-wrapper">
            <div class="left-side bottom-left-side">
                <?= Loc::getMessage(\KitRegions::moduleId.'_LOCATION') ?>
            </div>
        </td>
        <td width="60%" class="right-side-wrapper bottom-right-side-wrapper">
            <div class="right-side bottom-right-side">
                ...
                <script>
                    $(document).on('click', '#locationSave', function (e) {
                        ...
                        if (value.length == 10) {
                            $.ajax({
                                url: '/bitrix/admin/kit_regions_ajax.php',
                                cache: false,
...
$tabControl->Buttons([
    'disabled' => false,
    'back_url' => 'kit_regions.php?lang='.LANGUAGE_ID.'&site='.$_REQUEST['site'],
]);
