...
<div class="puzzle_block news-block__wrapper main-container stock-wrapper__left-navigation size">
    <p class="puzzle_block__title fonts__middle_title">
        <?=$arParams["BLOCK_NAME"]?>
        <a href="<?=($arResult["ITEMS"][0]["LIST_PAGE_URL"]) ? $arResult["ITEMS"][0]["LIST_PAGE_URL"] : $arParams["LINK_TO_THE_FULL_LIST"]?>" class="puzzle_block__link fonts__small_text" title="<?=Loc::getMessage("SOTBIT_PROMOTIONS_IMAGES_AND_LIST_LINK_TEXT");?>">
            <?=Loc::getMessage("SOTBIT_PROMOTIONS_IMAGES_AND_LIST_LINK_TEXT");?>
            <i class="icon-nav_1"></i>
        </a>
    </p>

    <div id="news_block_three_<?= $idItem ?>" class="news_block_three swiper-container">
        <div class="swiper-wrapper">
            ...
            <?foreach(array_slice($arResult["ITEMS"], 0, 7) as $arItem):?>
                ...
                ?>
                <?if($i <= 2):?>
                    <div class="news_block_three__item swiper-slide" id="<?=$this->GetEditAreaId($arItem['ID']);?>"  data-timer="timerID_<?=$blockID?>">
                        <?if($arParams["DISPLAY_PICTURE"]!="N"):?>
                            <?if(!$arParams["HIDE_LINK_WHEN_NO_DETAIL"] || ($arItem["DETAIL_TEXT"] && $arResult["USER_HAVE_ACCESS"])):?>
                                <a href="<?=$arItem["DETAIL_PAGE_URL"]?>" class="news_block_three__img_link <?=$hoverClass?>"  title="<?if(isset($arItem["IPROPERTY_VALUES"]["ELEMENT_META_TITLE"])) echo $arItem["IPROPERTY_VALUES"]["ELEMENT_META_TITLE"]; else echo $arItem["NAME"]?>">
                            ...
                            <?endif;?>
                        <?endif?>
                        <div class="news_block_three__content">
                            <?if($arParams["DISPLAY_NAME"]!="N" && $arItem["NAME"]):?>
                                <?if(!$arParams["HIDE_LINK_WHEN_NO_DETAIL"] || ($arItem["DETAIL_TEXT"] && $arResult["USER_HAVE_ACCESS"])):?>
                                    <p class="news_block_three__content_comment fonts__main_text">
                                ...
                                <?else:?>
                                    <p class="news_block_three__content_comment fonts__main_text"><?=$arItem["NAME"]?></p>
                                <?endif;?>
                            <?endif;?>
                            <?if($arParams["DISPLAY_PREVIEW_TEXT"]!="N" && $arItem["PREVIEW_TEXT"]):?>
                                <p class="news_block_three__content_comment fonts__small_text"><?=$arItem["PREVIEW_TEXT"];?></p>
                            <?endif;?>
                ...
                <?else:?>
                    <?if($i == 3):?>
                        <div class="news_block_three__others swiper-slide">
                            <p class="news_block_three__others_title"><?=Loc::getMessage("SOTBIT_PROMOTIONS_ADDITIONAL_TEXT");?></p>
                    <?endif;?>
...
<script>
    const news_block_three_<?= $idItem ?> = new CreateSlider({
        sliderContainer: '#news_block_three_<?=$idItem?>',
        sizeSliderInit: 768
    });
</script>
