...
if (!empty($arResult['ITEMS']))
{
			...
			foreach($arProp as $propId => $prop)
			{
				...
				if ('TEXT' == $prop['SHOW_MODE'])
				{
					$skuTemplate[$itemId][$propId]['SCROLL']['START'] = '<div class="bx_item_detail_size full" id="#ITEM#_prop_'.$propId.'_cont">'.
						'<span class="subscribe-item__params-name">'.htmlspecialcharsbx($prop['NAME']).'</span>'.
				...
				{
					$skuTemplate[$itemId][$propId]['SCROLL']['START'] = '<div class="bx_item_detail_scu full subscribe-item__detail_scu" id="#ITEM#_prop_'.$propId.'_cont">'.
						'<span class="subscribe-item__params-name">'.htmlspecialcharsbx($prop['NAME']).':</span>'.
					...
					$skuTemplate[$itemId][$propId]['FULL']['START'] = '<div class="bx_item_detail_scu subscribe-item__detail_scu" id="#ITEM#_prop_'.$propId.'_cont">'.
						'<span class="subscribe-item__params-name">'.htmlspecialcharsbx($prop['NAME']).':</span>'.
						'<div class="bx_scu_scroller_container"><div class="bx_scu"><ul id="#ITEM#_prop_'.$propId.'_list" style="width: #WIDTH#;">';
					$skuTemplate[$itemId][$propId]['FULL']['FINISH'] = '</ul></div>'.
						'<div class="bx_slide_left" id="#ITEM#_prop_'.$propId.'_left" data-treevalue="'.$propId.'" style="display: none;"></div>'.
					...
					foreach ($prop['VALUES'] as $value)
					{
                        if($prop[CODE] == Config::get('COLOR')) {
                            $value['NAME'] = htmlspecialcharsbx($value['NAME']);
                            $skuTemplate[$itemId][$propId]['ITEMS'][$value['ID']] = '<li data-treevalue="'.$propId.'_'.$value['ID'].
                                '" data-onevalue="'.$value['ID'].'" data-name="color"><i title="'.$value['NAME'].'"></i>'.
                                '<span class="subscribe-item__img"><span class="cnt_item" style="background-image:url(\''.$value['PICT']['SRC'].'\');" title="'.$value['NAME'].'"></span></span></li>';
                        } else {
                            $value['NAME'] = htmlspecialcharsbx($value['NAME']);
                            $skuTemplate[$itemId][$propId]['ITEMS'][$value['ID']] = '<li data-treevalue="'.$propId.'_'.$value['ID'].
                                '" data-onevalue="'.$value['ID'].'" data-name="property"><span title="'.$value['NAME'].'">'.$value['NAME'].'</span>'.
                                '<span class="subscribe-item__params"><span class="cnt_item" title="'.$value['NAME'].'"></span></span></li>';
                        }
	...
	<? foreach ($arResult['ITEMS'] as $key => $arItem)
	{
		...
		?>
	<div class="<?=($arItem['SECOND_PICT'] && $arParams ? 'subscribe-item double' : 'subscribe-item');?>" id=
		"<?=$strMainID;?>">
    <div class="subscribe-item__wrapper">
        <?if($arItem['CAN_BUY']):?>
            <div class="subscribe-item__availability">                                              <!--availability-->
                <?=Loc::getMessage('CPSL_SUBSCRIBE_AVAILABILITY');?>
            </div>
        <?else:?>
            <div class="subscribe-item__not-availability">                                          <!--NOT availability-->
                <?=Loc::getMessage('CPSL_SUBSCRIBE_NOT_AVAILABILITY');?>
            </div>
        <?endif;?>
<!--		<div class="catalog-product-subscribe-list-close">-->
<!--			<span class="catalog-product-subscribe-list-close-item"></span>-->
<!--		</div>-->
	<div class="subscribe-item__container <?=$showImgClass;?>">
	<a id="<?=$arItemIDs['PICT'];?>" href="<?=$arItem['DETAIL_PAGE_URL'];?>" class=
		"subscribe-item__images"<? if ($arParams['SHOW_IMAGE'] == "Y")
		{
	...
	<div class="subscribe-item_price">
    ...
    {
        ?>
        <div class="subscribe-item_articul" style="display: none">
            <?
            foreach ($arItem['DISPLAY_PROPERTIES'] as $arOneProp)
            {
                ?>
                <div class="<?= $arOneProp[CODE]?>">
    ...
    <?
		...
		?>
        <div class="subscribe-item__containt">
		    <div class="subscribe-item__controls 123">
			<div>
				<a class="subscribe-item__btn-more-details" href="<?=$arItem['DETAIL_PAGE_URL'];?>">
					<?=GetMessage('CPSL_TPL_MESS_BTN_DETAIL');?>
				</a>
			</div>
			<div>
				<a id="<?=$arItemIDs['SUBSCRIBE_DELETE_LINK'];?>" class="subscribe-item__btn-unsubscribe main-color-btn-hover-fill" href="javascript:void(0)">
    ...
    <div class="subscribe-item__containt"><?
        ...
        {
        ?>
            <div class="subscribe-item_articul-sku" style="display: none">
                <?
    ...
    <script>
        function setMinHeight (item) {
            let height = item.offsetHeight;
            item.style.minHeight = height + 'px';
        }

        function resetMinHeight (item) {
            item.style.minHeight = 'auto';
        }
        let items = Array.prototype.slice.call(document.getElementsByClassName('subscribe-item'));
        if (items.length > 0) {
            items.forEach(setMinHeight);
            window.addEventListener('resize', function () {
                items.forEach(resetMinHeight);
                items.forEach(setMinHeight);
            });
        }
    </script>
