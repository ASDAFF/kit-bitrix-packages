...
<div class="promotion-block__wrapper puzzle_block main-container size">
    <p class="puzzle_block__title fonts__middle_title">
        ...
        <a href="<?= ($arResult["ITEMS"][0]["LIST_PAGE_URL"]) ? $arResult["ITEMS"][0]["LIST_PAGE_URL"] : $arParams["LINK_TO_THE_FULL_LIST"] ?>"
           title="<?= Loc::getMessage("KIT_PROMOTIONS_VERTICAL_LINK_TEXT"); ?>"
           class="puzzle_block__link fonts__small_text">
            <?= Loc::getMessage("KIT_PROMOTIONS_VERTICAL_LINK_TEXT"); ?>
            <i class="icon-nav_1"></i>
        </a>
    </p>

    <div id="promotion_block_<?= $idItem ?>" class="promotion_block swiper-container">
        <div class="promotion_block__wrapper swiper-wrapper">
            <? foreach (array_slice($arResult["ITEMS"], 0, 5) as $arItem):
                ...
                ?>
                <div class="promotion_block__content swiper-slide" id="<?= $this->GetEditAreaId($arItem['ID']); ?>"
...
<script>
    const promotion_block_<?= $idItem ?> = new CreateSlider({
        sliderContainer: '#promotion_block_<?=$idItem?>',
        sizeSliderInit: 768
    });
</script>
