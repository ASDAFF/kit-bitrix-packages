...
JCSmartFilter.prototype.reload = function(input)
{
	...
	if (this.form)
	{
		...
		this.inputValues = [];
        for (prop in values) {
            if ((values[prop].name.indexOf("searchFilter") != -1) || (values[prop].name.indexOf("brandFilter") != -1)) {
		...
		else
		{
			if (this.sef)
			{
				var set_filter = document.getElementsByClassName("set_filter");/*BX('set_filter')*/;
...
JCSmartFilter.prototype.bindUrlToButton = function (buttonId, url)
{
	...
	if (button)
	{
		for(var i=0;i<button.length;++i)
		{
			...
			BX.bind(button[i], 'click', proxy(url, function(url)
			{
...
JCSmartFilter.prototype.hideFilterProps = function(element)
{
	...
	if (mql.matches) { // if filter is mobile
		...
		var angleUp = "icon-nav_button-rotate";
		var angleDown = "icon-nav_button";
		...
		if(BX.hasClass(obj, "active"))
		{
			...
			BX.addClass(propAngle, angleDown);
			BX.removeClass(propAngle, angleUp);
		...
		{
			var activeWrapper = ".bx_filter .catalog_content__filter_horizon_item.active";
			$(activeWrapper + " [data-role='bx_filter_block']").hide();
			$(activeWrapper + " [data-role='prop_angle']").addClass(angleDown).removeClass(angleUp);
			...
			BX.removeClass(propAngle, angleDown);
			BX.addClass(propAngle, angleUp);
			...
			function getPropsPosition() {
				...
				let element = document.querySelector(".catalog_content__filter_horizon_item.active .catalog_content__filter_horizon_item_wrapper");
				let elementCoords = element.getBoundingClientRect();

				if (elementCoords.right > windowWidth) {
					BX.addClass(element, "wrapper_right");
				} else {
					BX.removeClass(element, "wrapper_right")
				}
			}
		}
	...
	function resizeDropDown(id) {
		...
		function setDropdownPosition() {
			if (document.getElementById(id)) {
				...
				if (mql.matches) {		//mobile dropdown

					let clientHeight = document.querySelector(".bx_filter").clientHeight;
					let liItems = thisDropDown.querySelectorAll(".bx-filter-select-popup ul li");
					...
					for (let i = 1; i < liItems.length; i++) {
						liItems[i].style.borderBottom = "none";
					}

					thisDropDown.style.width = "220px";
...
function searchfieldRefresh() {
	$(".catalog_content__filter_horizon_item").each(function(i, v)
	{
...
function filterList(header, list) {
    $(header).change(function () {
        let filter = $(header).val().toLowerCase();

        if (filter) {
            let $matches = $(list).find('label').parent();
            let array = [];
...
{
	var allCheckboxes = $(element).closest('.catalog_content__filter_horizon_item_inner').find('input[type="checkbox"]:not([disabled])');
...
function checkToggleResetAll(element) {
    var allCheckboxes = $(element).find('input[type="checkbox"]');
    var allRadios = $(element).find('input[type="radio"]');
        allCheckboxes.prop('checked', true).click();
...
$(document).mouseup(function(e) {
	...
	var activeWrapperClass = ".bx_filter .catalog_content__filter_horizon_item.active";
	var activeWrapper = $(activeWrapperClass);
	var dropdown = $(activeWrapperClass + " [data-role='bx_filter_block']");

	var popupSelect = $(".popup-window");
	...
	if(!activeWrapper.is(e.target) && activeWrapper.has(e.target).length === 0 && !popupSelect.is(e.target) && popupSelect.has(e.target).length === 0) {
		dropdown.hide();
		// $(activeWrapperClass + " [data-role='prop_angle']").addClass(angleDown).removeClass(angleUp);
		// $(activeWrapperClass).removeClass("active");
		if (document.querySelector(activeWrapperClass)) {
			let wrapper = document.querySelector(activeWrapperClass);
			let element = wrapper.querySelector(".catalog_content__filter_horizon_item_name");
    ...
    function setHorizontalFilterEvents() {
        ...
        function colorizeFilterUsedItem(item) {
            if (isFilterItemUsed(item)) {
                ...
                if (oldClass.indexOf("filter_used") === -1) {
                    item.setAttribute("class", oldClass + " filter_used");
                }

                let navIcon = item.querySelector("[data-role='prop_angle']");
                let iconCancel = item.querySelector(".horizontal_filter-icon_cancel");
                ...
                let oldClass = item.getAttribute("class");
                let newClass = oldClass.replace(" filter_used", "");
        ...
        function displayResetAllFiltersButton(items) {
            let resetAllFiltersButton = document.querySelector(".filter_horizontal-reset-all-filters");

            if (isSomeItemUsed(items)) {
                resetAllFiltersButton.style.visibility = "visible";
            } else {
                resetAllFiltersButton.style.visibility = "hidden";
            }
        }
        ...
        function isFilterItemUsed(item) {
            ...
            let inputs = item.querySelectorAll(".bx_filter_input_container > input");

            if ((innerItem.textContent.length > 0)
        ...
        function showSelectedNumbers() {
            if (!mql.matches) {
				let items = document.querySelectorAll(".catalog_content__filter_horizon_item");

                for (let i = 0; i < items.length; i++) {
                    let numberSelectedItemsBlock = items[i].querySelector(".number_selected_items");
                    let numberSelectedItems = getCheckedCheckboxesNumber(items[i]);
                    let itemName = items[i].querySelector(".item_name");
        ...
        function getCheckedCheckboxesNumber(item) {
            ...
            for (let i = 0; i < checkBoxes.length; i++) {
                count = checkBoxes[i].checked ? ++count : count;
            }
        ...
        function reset(items) {
            for (let i = 0; i < items.length; i++) {
                let navIcon = items[i].querySelector("[data-role='prop_angle']");
                let iconCancel = items[i].querySelector(".horizontal_filter-icon_cancel");
                let numberSelectedItems = items[i].querySelector(".number_selected_items");
                let oldClass = items[i].getAttribute("class");
                let newClass = oldClass.replace(" filter_used", "");
                let innerWrapper = items[i].querySelector(".catalog_content__filter_horizon_item_wrapper");

                navIcon.style.visibility = "initial";
                iconCancel.style.display = "none";
                numberSelectedItems.textContent = "";
                items[i].setAttribute("class", newClass);
        ...
        window.resetSlideTracks = function (item) {
            if (item.querySelector(".filter-slide_tracks-wrapper")) {
                let filterSlideTracksWrapper = item.querySelector(".filter-slide_tracks-wrapper"),
        ...
        function hideFilterProps() {
            if (document.querySelector(".bx_filter .catalog_content__filter_horizon_item.active")) {
                let activeWrapperClass = ".bx_filter .catalog_content__filter_horizon_item.active";
                let dropdown = $(activeWrapperClass + " [data-role='bx_filter_block']");
        ...
        function setEventListeners() {
            let itemsInner = document.querySelectorAll(".catalog_content__filter_horizon_item_wrapper");
            let items = document.querySelectorAll(".catalog_content__filter_horizon_item");
            ...
            for (let i = 0; i < items.length; i++) {
                let iconClose = items[i].querySelector(".horizontal_filter-icon_cancel");
