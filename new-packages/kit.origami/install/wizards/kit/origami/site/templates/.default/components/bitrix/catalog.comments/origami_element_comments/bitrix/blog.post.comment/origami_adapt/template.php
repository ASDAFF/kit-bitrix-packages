...
<ul class="blog-comments reviews-product" id="blg-comment-<?=$arParams["ID"]?>">
<a name="comments"></a>
<?
...
{
	?>
	<div class="blog-textinfo blog-note-box">p
		...
		function ShowComment($comment, $tabCount=0, $tabSize=2.5, $canModerate=false, $User=Array(), $use_captcha=false, $bCanUserComment=false, $errorComment=false, $arParams = array())
		{
			...
			if($comment["SHOW_AS_HIDDEN"] == "Y" || $comment["PUBLISH_STATUS"] == BLOG_PUBLISH_STATUS_PUBLISH || $comment["SHOW_SCREENNED"] == "Y" || $comment["ID"] == "preview")
			{
				...
				elseif($prevTab > 10)
					...
					$prevTab = $tabCount;
				?>
                <div  data-count="<?=$tabCount?>">
                    <div class="blog-comment-line" style="margin-left:<?=$prevPaddingSize?>em;"></div>
                    <a name="<?=$comment["ID"]?>"></a>
                    <div class="reviews-product__message" style="padding-left:<?=$paddingSize?>em;">
                    <div id="blg-comment-<?=$comment["ID"]?>" class="reviews-product__message">
				...
				if($comment["PUBLISH_STATUS"] == BLOG_PUBLISH_STATUS_PUBLISH || $comment["SHOW_SCREENNED"] == "Y" || $comment["ID"] == "preview")
				{
					...
					<div class="reviews-product__author">
                        ...
                        <div class="reviews-product__avatar">
                            <?if( false && isset($comment['BlogUser']['Avatar_resized']['30_30']['src']) && file_exists($_SERVER['DOCUMENT_ROOT'].$comment['BlogUser']['Avatar_resized']['30_30']['src'])):?>
                                <img class="reviews-product__avatar-img" src="<?=$comment['BlogUser']['Avatar_resized']['30_30']['src']?>" alt="">
                            <?elseif(isset($comment['AVATAR_file']['SRC']) && file_exists($_SERVER['DOCUMENT_ROOT'].$comment['AVATAR_file']['SRC'])):?>
                                <img class="reviews-product__avatar-img" src="<?=$comment['AVATAR_file']['SRC']?>" alt="">
                            <?endif;?>
                        </div>
                        <div class="reviews-product__author-description">
                            <?
                            if(strlen($comment["urlToBlog"])>0)
                            {
                                ?>
                                <div class="reviews-product__author-name">
                                <?
                            ...
                            {
                                ?>
                                <div class="reviews-product__author-name"><?=$comment["AuthorName"]?></div>
                                <?
                            }
                            ...
                            ?>
                            <div class="reviews-product__message-date"><?=$comment["DateFormated"]?></div>
                        </div>
					</div>
					<div class="blog-clear-float"></div>
                    ...
                    <ul class="reviews-product__action-list">
						<?
						if($bCanUserComment===true)
						{
							?>
							<li class="blog-comment-answer reviews-product__action-item"><a href="javascript:void(0)" onclick="return replyCommentNew('<?=$comment["ID"]?>', '<?=$comment["POST_ID"]?>')"><?=GetMessage("B_B_MS_REPLY")?></a></li>
							<?
						}
						...
						{
							?>
							<li class="blog-comment-parent reviews-product__action-item"><a href="#<?=$comment["PARENT_ID"]?>"><?=GetMessage("B_B_MS_PARENT")?></a></li>
							<?
						}
						...
						if($comment["CAN_EDIT"] == "Y")
						{
							...
							<li class="blog-comment-edit reviews-product__action-item"><a href="javascript:void(0)" onclick="return editCommentNew('<?=$comment["ID"]?>', <?=$comment["POST_ID"]?>)"><?=GetMessage("BPC_MES_EDIT")?></a></li>
							<?
						}
						if(strlen($comment["urlToShow"])>0)
						{
							?>
							<li class="blog-comment-show reviews-product__action-item">
						...
						if(strlen($comment["urlToHide"])>0)
						{
							...
							<li class="blog-comment-show reviews-product__action-item">
								...
								<?endif;?>
								<?=GetMessage("BPC_MES_HIDE")?></a></li>
							<?
						}
						if(strlen($comment["urlToDelete"])>0)
						{
							?>
							<li class="blog-comment-delete reviews-product__action-item">
								<?if($arParams["AJAX_POST"] == "Y"):?>
								...
								<?endif;?>
								<?=GetMessage("BPC_MES_DELETE")?></a></li>
							<?
						}
						if(strlen($comment["urlToSpam"])>0)
						{
							?>
							<li class="blog-comment-delete blog-comment-spam reviews-product__action-item"><a href="<?=$comment["urlToSpam"]?>" title="<?=GetMessage("BPC_MES_SPAM_TITLE")?>"><?=GetMessage("BPC_MES_SPAM")?></a></li>
							<?
						}
						if ($arParams["SHOW_RATING"] == "Y")
						{
							?>
							<li class="rating_vote_text reviews-product__action-item">
							<?$GLOBALS["APPLICATION"]->IncludeComponent(
								"bitrix:rating.vote", $arParams["RATING_TYPE"],
								Array(
									"ENTITY_TYPE_ID" => "BLOG_COMMENT",
...
//							dbg showComment?
		...
		if($arResult["is_ajax_post"] != "Y")
		{
			if($arResult["CanUserComment"])
			{
				...
				?>
				<div class="blog-add-comment"><a class="main_btn" href="javascript:void(0)" onclick="return editCommentNew('0', <?=$arParams["ID"]?>)"><?=GetMessage("B_B_MS_ADD_COMMENT")?></a></div>
				<a name="0"></a>
				<?
			...
			{
				?>
				<ul class="reviews-product__pagination">
                    <li class="reviews-product__pagination-action prev<?=($arResult["PAGE"]==1) ? ' hidden-nav' : ''?>">
                        <a href="#" class="reviews-product__pagination-prev" onclick="return bcPrev(this);"><</a>
                    </li>
				<? for($i = 1; $i <= $arResult["PAGE_COUNT"]; $i++)
					{
						...
						if($i == $arResult["PAGE"])
                            $style_ext = " active";
						?>
                        <li class="reviews-product__pagination-page<?=$style_ext?>">
                            <a href="<?=$arResult["NEW_PAGES"][$i]?>" onclick="return bcNav('<?=$i?>', this)" id="blog-comment-nav-t<?=$i?>"><?=$i?></a>
                        </li>
                        <?
					}
				?>
                    <li class="reviews-product__pagination-action next<?=($arResult["PAGE"]==$arResult["PAGE_COUNT"] - 1) ? ' hidden-nav' : ''?>">
                        <a href="#" class="reviews-product__pagination-next" onclick="return bcNext(this);">></a>
                    </li>
				</ul>
				<?
			}
			...
			if($arResult["NEED_NAV"] == "Y")
			{
                ...
                <ul class="reviews-product__pagination">
					...
					for($i = 1; $i <= $arResult["PAGE_COUNT"]; $i++)
					{
                        ...
                        ?>
                        <li class="reviews-product__pagination-page<?=$style_ext?>">
                            <a href="<?=$arResult["NEW_PAGES"][$i]?>" onclick="return bcNav('<?=$i?>', this)" id="blog-comment-nav-b<?=$i?>"><?=$i?></a>
                        </li>
                        <?
					}
				?>
