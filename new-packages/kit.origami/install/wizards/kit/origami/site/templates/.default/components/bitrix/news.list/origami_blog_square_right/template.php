...
<div id="blog__wrapper_<?=$idItem?>" class="blog__wrapper puzzle_block main-container size">
        <p class="puzzle_block__title fonts__middle_title">
            <?=$arParams["BLOCK_NAME"]?>
            <a href="<?=($arResult["ITEMS"][0]["LIST_PAGE_URL"]) ? $arResult["ITEMS"][0]["LIST_PAGE_URL"] : $arParams["LINK_TO_THE_FULL_LIST"]?>" title="<?=Loc::getMessage("SOTBIT_BLOG_SQUARE_RIGHT_LINK_TEXT");?>" class="puzzle_block__link fonts__small_text">
                <?=Loc::getMessage("SOTBIT_BLOG_SQUARE_RIGHT_LINK_TEXT");?>
                <i class="icon-nav_1"></i>
            </a>
        </p>

        <div class="blog_block-square swiper-container">
            <div class="blog_block-square__wrapper swiper-wrapper">
                <?
                $i = 0;
                ?>
                <?foreach($arResult["ITEMS"] as $arItem):?>
                    ...
                    ?>
                    <?if($i <= 2):?>
                        <div class="blog_block-square__block swiper-slide" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
                    ...
                    <?else:?>
                        <div class="blog_block-square__block-big swiper-slide" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
                            <div class="blog_block-square__item">
                                <?if($arParams["DISPLAY_PICTURE"]!="N"):?>
                                    <?if(!$arParams["HIDE_LINK_WHEN_NO_DETAIL"] || ($arItem["DETAIL_TEXT"] && $arResult["USER_HAVE_ACCESS"])):?>
                                    ...
                                    <?else:?>
                                        <div class="blog_block-square__img_big <?=$hoverClass?>">
                                            <?if($arItem["PREVIEW_PICTURE"]):?>
                                                <img <?=$strLazyLoad?>
                                                    width="<?=$arItem["PREVIEW_PICTURE"]["WIDTH"]?>"
                                                    height="<?=$arItem["PREVIEW_PICTURE"]["HEIGHT"]?>"
                                ...
                                <div class="blog_block-square__content_dig">
                                    <?if($arParams["DISPLAY_NAME"]!="N" && $arItem["NAME"]):?>
                                        <?if(!$arParams["HIDE_LINK_WHEN_NO_DETAIL"] || ($arItem["DETAIL_TEXT"] && $arResult["USER_HAVE_ACCESS"])):?>
                                            <a href="<?=$arItem["DETAIL_PAGE_URL"]?>" class="blog_block-square__content_dig_title fonts__main_text">
                                                <?=$arItem["NAME"]?>
                                            </a>
                                        <?else:?>
                                            <?=$arItem["NAME"]?>
...
<script>
    const origamiBlogSquare_right__<?=$idItem?> = new CreateSlider ({
        sliderContainer: '#blog__wrapper_<?=$idItem?> .blog_block-square',
        sizeSliderInit: 768
    });
</script>
