...
<div id="news-block__wrapper_<?= $idItem ?>" class="puzzle_block news-block__wrapper main-container size">
    <p class="puzzle_block__title fonts__middle_title">
        ...
        <a href="<?= ($arResult["ITEMS"][0]["LIST_PAGE_URL"]) ? $arResult["ITEMS"][0]["LIST_PAGE_URL"] : $arParams["LINK_TO_THE_FULL_LIST"] ?>"
           ...
           title="<?= Loc::getMessage("KIT_BLOG_IMAGES_AND_LIST_LINK_TEXT"); ?>">
            <?= Loc::getMessage("KIT_BLOG_IMAGES_AND_LIST_LINK_TEXT"); ?>
            <i class="icon-nav_1"></i>
        </a>
    </p>

    <div class="news_block_four swiper-container">
        <div class="news_block_four__wrapper swiper-wrapper">
            <div class="news_block_four__others swiper-slide">
                <p class="news_block_four__others_title"><?= Loc::getMessage("KIT_BLOG_ADDITIONAL_TEXT"); ?></p>
            ...
            <? foreach (array_slice($arResult["ITEMS"], 0, 7) as $arItem): ?>
                ...
                ?>
                <? if ($i <= 2): ?>
                    <div class="news_block_four__item swiper-slide" id="<?= $this->GetEditAreaId($arItem['ID']); ?>">
                        <? if ($arParams["DISPLAY_PICTURE"] != "N"): ?>
                            <? if (!$arParams["HIDE_LINK_WHEN_NO_DETAIL"] || ($arItem["DETAIL_TEXT"] && $arResult["USER_HAVE_ACCESS"])): ?>
                                <a href="<?= $arItem["DETAIL_PAGE_URL"] ?>"
                                   class="news_block_four__img_link <?= $hoverClass ?>"
                                   title="<? if (isset($arItem["IPROPERTY_VALUES"]["ELEMENT_META_TITLE"])) echo $arItem["IPROPERTY_VALUES"]["ELEMENT_META_TITLE"]; else echo $arItem["NAME"] ?>">
                            ...
                            <? endif; ?>
                        <? endif ?>
                        <div class="news_block_four__content">
                            <? if ($arParams["DISPLAY_NAME"] != "N" && $arItem["NAME"]): ?>
                                <? if (!$arParams["HIDE_LINK_WHEN_NO_DETAIL"] || ($arItem["DETAIL_TEXT"] && $arResult["USER_HAVE_ACCESS"])): ?>
                                    <p class="news_block_four__content_comment fonts__main_text">
                                        <a href="<?= $arItem["DETAIL_PAGE_URL"] ?>"
                                           title="<? if (isset($arItem["IPROPERTY_VALUES"]["ELEMENT_META_TITLE"])) echo $arItem["IPROPERTY_VALUES"]["ELEMENT_META_TITLE"]; else echo $arItem["NAME"] ?>">
                                            <?= $arItem["NAME"] ?>
                                        </a>
                                ...
                                <? else: ?>
                                    <p class="news_block_four__content_comment fonts__main_text"><?= $arItem["NAME"] ?></p>
                                <? endif; ?>
                            <? endif; ?>
                            <? if ($arParams["DISPLAY_PREVIEW_TEXT"] != "N" && $arItem["PREVIEW_TEXT"]): ?>
                                <p class="news_block_four__content_comment fonts__small_text"><?= $arItem["PREVIEW_TEXT"]; ?></p>
                            <? endif; ?>
...
<script>
    const news_block_four_<?=$idItem?> = new CreateSlider({
        sliderContainer: '#news-block__wrapper_<?=$idItem?> .news_block_four',
        sizeSliderInit: 768
    });
</script>
