...
if(strlen($arResult["ID"])>0)
{
	...
	<form method="post"  class="sale-profile-detail-form" action="<?=POST_FORM_ACTION_URI?>" enctype="multipart/form-data">
        <div class="sale-personal-profile-info-block">
		<?=bitrix_sessid_post()?>
		...
		<div class="sale-personal-profile-btn-block">
			<input type="submit" class="btn main-profile-submit" name="save" value="<?echo GetMessage("SALE_SAVE") ?>">
			&nbsp;
			<input type="submit" class="btn main-profile-submit"  name="apply" value="<?=GetMessage("SALE_APPLY")?>">
			&nbsp;
			<input type="submit" class="btn main-profile-submit-cansel"  name="reset" value="<?echo GetMessage("SALE_RESET")?>">
		</div>
	</form>
